...
function showItems(player, unit, offset)
    ...
    for i = temp, temp+maxGossipItems-1 do
        ...
        player:GossipMenuAddItem(4, v[2], safeGossipItems+offset, v[1]+1)
    end
    if(temp+maxGossipItems-1 < #E) then
        player:GossipMenuAddItem(7, "Далее..", safeGossipItems+offset+1, 0)
    end
    player:GossipMenuAddItem(7, "Назад..", safeGossipItems+offset-1, 0)
    player:GossipSendMenu(100, unit, menu_id)
end
    
function HELLO(event, player, unit)
    player:GossipMenuAddItem(9, "Правая рука", 0, EQUIPMENT_SLOT_MAINHAND)
    player:GossipMenuAddItem(9, "Левая рука", 0, EQUIPMENT_SLOT_OFFHAND)
    player:GossipSendMenu(100, unit, menu_id)
end
    ...
    elseif(sender >= safeGossipItems) then
        if(action ~= 0) then
            ...
            if(D[guid].slot and D[guid].display) then
                ...
                if(not item) then
                    player:SendNotification("В этом слоте нет предмета.")
                else
